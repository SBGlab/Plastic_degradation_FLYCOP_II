...
echo "biomass1\tbiomass2\tgly1\tgly2\to2\tfitness" > $dataAnalysisDir/tableParamWithFitness.csv
cut -d, -f1-7,9 $dataAnalysisDir/paramstrings_withFitness.csv | awk 'BEGIN{FS="[=,]"} {print $2"\t"$4"\t"$6"\t"$8"\t"$10"\t"$12"\t"$14"\t"$15}' | sed "s/'//g" | sed "s/-999999999/0/">> $dataAnalysisDir/tableParamWithFitness.csv
...
python3 -W ignore ../../Scripts/plasticSimulation_individualTestFLYCOP.py $param1 $param2 $param3 $param4 $param5 $fitness
