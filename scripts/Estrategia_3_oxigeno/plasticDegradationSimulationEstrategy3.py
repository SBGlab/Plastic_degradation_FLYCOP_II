...
def model_modifications(gly2, model_id, gly1,o2): #Quitamos gly=None
    # Single GEMs parameter modifications

    # 1.1.- Establish modifications in model  
    model1=c.model('/home/iodmc/Documents/FLYCOP-ox_strategy3/Scripts/plasticDegradationEstrategy3-dPCA_10.cmd')
    ...
    model1.change_bounds('EX_o2_e',o2,0) #ox
    model1.change_bounds('tpha_e',-5,0) #modificado por isa para ver si por esto me da diferente que a Ana
    
    model1.id=model_id[0]
    model1.write_comets_model()
    del(model1)
     # 1.2.- Establish modifications in model 2
    model2=c.model('/home/iodmc/Documents/FLYCOP-ox_strategy3/Scripts/defaultModel-3-consumoSeparado_10.cmd')
    model2.change_bounds('EX_glycol_e', gly2, 0)
    model2.change_bounds('EX_o2_e',o2,0) #ox
...
def Flycop(gly2,biomass1,biomass2,gly1,o2,fitFunc='Yield', dirPlot='', repeat=10, params_package=None, params_global=None):
    ...
    layout_file='/home/iodmc/Documents/FLYCOP-ox_strategy3/Scripts/layout_3.txt'
    ...
    if not(os.path.exists('/home/iodmc/Documents/FLYCOP-ox_strategy3/Scripts/noPCADeg_tmp.cmd')):
        
        model_modifications(gly2, model_id, gly1,o2)
    ...
    for i in range(repeat):
        ...
        if(dirPlot != ''):
            file2=dirPlot+'biomass_vs_tpha_'+'_run'+str(i)+'_'+str(round(fitness,6))+'_'+str(biomass1)+'_'+str(biomass2)+'_'+str(gly1)+'_'+str(gly2)+'_'+str(o2)+'.pdf'
    ...
    print("Fitness_function\tconfiguration(o2,gly1,gly2,bmass1,bmass2)\tfitness\tsd\tavg.Biomass\tavg.Yield\tendCycle")
                    
    print(fitFunc+"\t"+str(o2)+','+str(gly2)+','+str(gly1)+','+str(biomass1)+','+str(biomass2)+','+"\t"+str(round(avgfitness,6))+"\t"+str(sdfitness)+"\t"+str(round(avgBiomass,6))+"\t"+str(round(avgYield,6))+"\t"+str(endCycle))
    with open(dirPlot+"configurationsResults"+fitFunc+".txt", "a") as myfile:
        ...
        myfile.write(fitFunc+"\t"+str(gly2)+','+str(gly1)+','+str(biomass1)+','+str(biomass2)+','+str(o2)+','+"\t"+str(round(avgfitness,6))+"\t"+str(sdfitness)+"\t"+str(round(avgBiomass,6))+"\t"+str(round(avgYield,6))+"\t"+str(endCycle)+"\n")
