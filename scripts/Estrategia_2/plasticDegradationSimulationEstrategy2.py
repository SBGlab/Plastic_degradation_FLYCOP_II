...
def model_modifications(tpha,model_id, gly=None, dhbz=None):
    ...
    model1.change_bounds('EX_o2_e', -15, 0)
...
def compute_fitness(biomassYieldNew, finalBiomass, iniBiomass, fitFunc, fitTime, comets,df2):

    if(fitFunc=='Yield'):
        phaProduction=float(df2.iloc[-2]['C80aPHA_e'])+float(df2.iloc[-2]['C80aPHA_default_e'])
...
def Flycop(tpha,biomass1,biomass2, gly, fitFunc='Yield', dirPlot='', repeat=10, params_package=None, params_global=None):
    ...
    strains=['Pseudomonas1-dEG', 'Pseudomonas2-WT']
    ...
    for i in range(repeat):
        ...
        fitness= compute_fitness(biomassYieldNew, finalBiomass, iniBiomass, fitFunc, fitTime, comets, df2)
